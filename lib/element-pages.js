...
const elementStories = require('../data/element-stories.js');
...
function editorialSections(e) {
  const story = elementStories[e.number - 1];
  if (!story) throw new Error(`Missing static editorial story for element ${e.number}`);
  return `<section class="element-editorial"><h2>${escape(story.title)}</h2><p>${escape(story.body)}</p></section>`;
}
...
function detail(e) {
  ...
  return `<main class="page-container element-detail">
    ...
    <section class="element-sources"><h2>Sources &amp; further reading</h2><p><a href="https://pubchem.ncbi.nlm.nih.gov/element/${e.number}">PubChem: ${e.name}</a> · <a href="${escape(e.source)}">${e.name} on Wikipedia</a></p><p>The introduction and original editorial story are adapted from Wikipedia contributors via <a href="https://github.com/Bowserinator/Periodic-Table-JSON">Periodic-Table-JSON</a>. Element descriptions and their adaptations are available under <a href="https://creativecommons.org/licenses/by-sa/3.0/">CC BY-SA 3.0</a>. Numerical attributes use the <a href="https://pubchem.ncbi.nlm.nih.gov/rest/pug/periodictable/JSON">PubChem periodic-table dataset</a>, retrieved September 22, 2026.</p></section>
